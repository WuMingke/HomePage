package com.erkuai.homepage.widget;
...
//                }
    ...
    private List<View> getStickyChildren(View view) {
        List<View> children = new ArrayList<>();

        List<View> allChildViews = getAllChildViews(view);
        for (int i = 0; i < allChildViews.size(); i++) {
            View child = allChildViews.get(i);
    ...
    private void resetSticky() {
        List<View> children = getStickyChildren(this);
        if (!children.isEmpty()) {
            int count = children.size();
            // 让所有的View恢复原来的状态
            for (int i = 0; i < count; i++) {
